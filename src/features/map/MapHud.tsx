...
// 2D↔3D 전환 — 행사모드는 라우트 이동(href), 체험모드는 화면 내 로컬 토글(onClick).
//   둘 중 하나만 주면 된다(href 우선). 미전달이면 전환 버튼 자체를 렌더하지 않는다.
export type ModeSwitch = { label: string; href?: string; onClick?: () => void };
...
export function MapHud({
  ...
  modeSwitch?: ModeSwitch; // 2D↔3D 전환(행사=라우트, 체험=로컬 토글). 미전달이면 숨김
  ...
  return (
    <>
      ...
      {/* 우상단: 2D↔3D 전환 — 행사=라우트 이동(Link), 체험=로컬 토글(button). 둘 다 동일 스타일. */}
      {modeSwitch &&
        (modeSwitch.href ? (
          <Link
          ...
          <button
            type="button"
            onPointerDown={stop}
            onClick={modeSwitch.onClick}
            className={`absolute right-[max(0.75rem,var(--spacing-safe-r))] top-[max(0.75rem,var(--spacing-safe-t))] z-10 flex items-center gap-1 rounded-full px-4 py-2.5 text-sm font-extrabold ${MAP_GOLD_BUTTON}`}
            style={{ minHeight: 0 }} // 전역 button{min-height:48px} 무력화(도감 버튼과 동일)
          >
            {modeSwitch.label}
          </button>
        ))}
