...
export function enterFullscreen(): void {
  ...
  if (document.fullscreenElement) {
    lockLandscape(); // 이미 전체화면이어도 가로 잠금은 시도
    return;
  }
  ...
  try {
    ...
    if (r && typeof (r as Promise<void>).then === "function") {
      // 표준: 전체화면 확정 후 가로 잠금(orientation.lock 은 전체화면 상태에서만 동작 — Android).
      (r as Promise<void>).then(lockLandscape).catch(() => {});
    } else {
      // 구형(webkit, 프라미스 없음): 전체화면 반영을 잠깐 기다렸다 가로 잠금 시도.
...
/**
 * 화면을 가로(landscape)로 고정한다. Android 크롬 등은 **전체화면 상태에서만** 잠금이 허용된다.
 *   iOS Safari 는 Screen Orientation lock 미지원 → 조용히 무시(홈 화면 추가 PWA + manifest 가로 대체).
 *   미지원/거부는 무시(세로 회전 안내 오버레이 PortraitGuard 가 폴백).
 */
