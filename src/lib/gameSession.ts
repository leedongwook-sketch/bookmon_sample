...
// sessionStorage에 저장되는 게임 상태의 데이터 부분 (gameStore persist 와 동일 구조·저장소)
...
/**
 * sessionStorage("bookmon-game")를 읽어 저장된 게임 상태를 반환.
 * zustand persist 래퍼({ state, version })와 순수 객체 둘 다 지원.
 * 키가 없거나 JSON 파싱 실패 시 null. (gameStore 와 동일 저장소 = sessionStorage)
 */
