...
/**
 * 게임 전역 상태 저장소.
 * - persist 미들웨어로 sessionStorage("bookmon-game")에 자동 영속 → 같은 탭 세션(내 AR 내비 왕복 포함) 동안 유지, 탭 닫으면 초기화.
...
export const useGameStore = create<GameState>()(
  persist(
    ...
    {
      name: GAME_STORAGE_KEY, // sessionStorage 키
      // 세션 단위 영속 — 탭/앱 세션 동안만 유지되고 탭을 닫으면 초기화된다.
      //   (AR 은 sessionStorage 핸드셰이크를 쓰는데, 같은 저장소라 세션 경계가 일치한다.)
